...
// TODO this setup means 1 adc can go to 1 pin only and thus only 3 adc pins can be inited need to
// setup multi sampling indepently
...
uint32_t AnalogIn::read() { return HAL_ADC_GetValue(&ADC_InitStruct); }

uint16_t AnalogIn::read_u16() { return (uint16_t)HAL_ADC_GetValue(&ADC_InitStruct); }

float AnalogIn::getVoltage() { return (float)(HAL_ADC_GetValue(&ADC_InitStruct)) * ADC_RATIO; }

float AnalogIn::getPercent() { return (float)(HAL_ADC_GetValue(&ADC_InitStruct)) / ADC_MAX; }
