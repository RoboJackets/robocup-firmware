...
void digitalout_deinit(pin_name pin) { HAL_GPIO_DeInit(pin.port, pin.pin); }

void digitalout_write(pin_name pin, int state) { HAL_GPIO_WritePin(pin.port, pin.pin, state != 0); }

void digitalout_toggle(pin_name pin) { HAL_GPIO_TogglePin(pin.port, pin.pin); }

int digitalout_read(pin_name pin) { return HAL_GPIO_ReadPin(pin.port, pin.pin); }
