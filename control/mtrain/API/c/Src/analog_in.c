...
void analogin_init(adc_pin_name pin, adc_handle* handle) {
    ...
    // TODO Possibly give init back to the user by having these just be a default by checking if
    // field is zeroed
...
// reads in a float
uint32_t analogin_read(adc_handle* handle) { return HAL_ADC_GetValue(&handle->ADC_InitStruct); }
