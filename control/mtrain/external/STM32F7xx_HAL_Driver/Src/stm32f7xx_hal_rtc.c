...
HAL_StatusTypeDef HAL_RTC_SetTime(RTC_HandleTypeDef* hrtc, RTC_TimeTypeDef* sTime,
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        tmpreg = (((uint32_t)(sTime->Hours) << 16) | ((uint32_t)(sTime->Minutes) << 8) |
                  ((uint32_t)sTime->Seconds) | ((uint32_t)(sTime->TimeFormat) << 16));
    }
...
HAL_StatusTypeDef HAL_RTC_SetDate(RTC_HandleTypeDef* hrtc, RTC_DateTypeDef* sDate,
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        datetmpreg = (((uint32_t)RTC_ByteToBcd2(sDate->Year) << 16) |
                      ...
                      ((uint32_t)RTC_ByteToBcd2(sDate->Date)) | ((uint32_t)sDate->WeekDay << 13));
    } else {
        assert_param(IS_RTC_YEAR(RTC_Bcd2ToByte(sDate->Year)));
        assert_param(IS_RTC_MONTH(RTC_Bcd2ToByte(sDate->Month)));
        assert_param(IS_RTC_DATE(RTC_Bcd2ToByte(sDate->Date)));

        datetmpreg = ((((uint32_t)sDate->Year) << 16) | (((uint32_t)sDate->Month) << 8) |
                      ((uint32_t)sDate->Date) | (((uint32_t)sDate->WeekDay) << 13));
    }
 ...
 * values. Reading RTC current time locks the values in calendar shadow registers until Current date
 * is read.
 * @retval HAL status
 */
...
HAL_StatusTypeDef HAL_RTC_SetAlarm(RTC_HandleTypeDef* hrtc, RTC_AlarmTypeDef* sAlarm,
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        tmpreg =
            (((uint32_t)(sAlarm->AlarmTime.Hours) << 16) |
             ((uint32_t)(sAlarm->AlarmTime.Minutes) << 8) | ((uint32_t)sAlarm->AlarmTime.Seconds) |
             ((uint32_t)(sAlarm->AlarmTime.TimeFormat) << 16) |
