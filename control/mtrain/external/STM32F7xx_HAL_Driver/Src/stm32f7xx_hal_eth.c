...
HAL_StatusTypeDef HAL_ETH_ConfigMAC(ETH_HandleTypeDef* heth, ETH_MACInitTypeDef* macconf) {
    ...
    if (macconf != NULL) {
        ...
        tmpreg |= (uint32_t)(
            macconf->Watchdog | macconf->Jabber | macconf->InterFrameGap | macconf->CarrierSense |
        ...
        tmpreg |= (uint32_t)((macconf->PauseTime << 16) | macconf->ZeroQuantaPause |
...
HAL_StatusTypeDef HAL_ETH_ConfigDMA(ETH_HandleTypeDef* heth, ETH_DMAInitTypeDef* dmaconf) {
    ...
    tmpreg |= (uint32_t)(dmaconf->DropTCPIPChecksumErrorFrame | dmaconf->ReceiveStoreForward |
    ...
    (heth->Instance)->DMABMR = (uint32_t)(
        dmaconf->AddressAlignedBeats | dmaconf->FixedBurst |
        dmaconf->RxDMABurstLength | /* !! if 4xPBL is selected for Tx or Rx it is applied for the
                                       other */
        dmaconf->TxDMABurstLength |
        dmaconf->EnhancedDescriptorFormat | (dmaconf->DescriptorSkipLength << 2) |
...
static void ETH_MACDMAConfig(ETH_HandleTypeDef* heth, uint32_t err) {
    ...
    tmpreg |= (uint32_t)(macinit.Watchdog | macinit.Jabber | macinit.InterFrameGap |
    ...
    (heth->Instance)->MACFFR = (uint32_t)(
        macinit.ReceiveAll | macinit.SourceAddrFilter | macinit.PassControlFrames |
    ...
    tmpreg |= (uint32_t)(dmainit.DropTCPIPChecksumErrorFrame | dmainit.ReceiveStoreForward |
    ...
    (heth->Instance)->DMABMR = (uint32_t)(
        dmainit.AddressAlignedBeats | dmainit.FixedBurst |
        dmainit.RxDMABurstLength | /* !! if 4xPBL is selected for Tx or Rx it is applied for the
                                      other */
        dmainit.TxDMABurstLength |
        dmainit.EnhancedDescriptorFormat | (dmainit.DescriptorSkipLength << 2) |
...
static void ETH_MACAddressConfig(ETH_HandleTypeDef* heth, uint32_t MacAddr, uint8_t* Addr) {
    ...
    tmpreg =
        ((uint32_t)Addr[3] << 24) | ((uint32_t)Addr[2] << 16) | ((uint32_t)Addr[1] << 8) | Addr[0];
