...
#define UTILS_MAX_FREQUENCY_SCALE1 \
    216000000U /*!< Maximum frequency for system clock at power scale1, in Hz */
#define UTILS_MAX_FREQUENCY_SCALE2 \
    180000000U /*!< Maximum frequency for system clock at power scale2, in Hz */
#define UTILS_MAX_FREQUENCY_SCALE3 \
    144000000U /*!< Maximum frequency for system clock at power scale3, in Hz */
...
#define UTILS_SCALE1_LATENCY6_FREQ                                                               \
    180000000U /*!< HCLK frequency to set FLASH latency 6 in power scale 1  with over-drive mode \
                */
#define UTILS_SCALE1_LATENCY7_FREQ                                                               \
    210000000U /*!< HCLK frequency to set FLASH latency 7 in power scale 1  with over-drive mode \
                */
...
#define UTILS_SCALE2_LATENCY5_FREQ \
    150000000U /*!< HCLK frequency to set FLASH latency 5 in power scale 2  */
...
#define UTILS_SCALE3_LATENCY3_FREQ \
    90000000U /*!< HCLK frequency to set FLASH latency 3 in power scale 3  */
#define UTILS_SCALE3_LATENCY4_FREQ \
    120000000U /*!< HCLK frequency to set FLASH latency 4 in power scale 3  */
/**
 * @}
 */
...
/** @addtogroup UTILS_EF_SYSTEM
 ...
 voltage range   |
             (++) |                |    2.7V - 3.6V    |    2.4V - 2.7V    |    2.1V - 2.7V
 |    1.8V - 2.1V    |
             (++)
 |----------------|-------------------|-------------------|-------------------|-------------------|
             (++) |0WS(1CPU cycle) |   0 < HCLK <= 30  |   0 < HCLK <= 24  |   0 < HCLK <= 22  |   0
 < HCLK <= 20  |
             (++)
 |----------------|-------------------|-------------------|-------------------|-------------------|
 ...
 < HCLK <= 100 |
             (++)
 |----------------|-------------------|-------------------|-------------------|-------------------|
             (++) |5WS(6CPU cycle) | 150 < HCLK <= 180 | 120 < HCLK <= 144 | 110 < HCLK <= 132 | 100
 < HCLK <= 120 |
             (++)
 |----------------|-------------------|-------------------|-------------------|-------------------|
             (++) |6WS(7CPU cycle) | 180 < HCLK <= 210 | 144 < HCLK <= 168 | 132 < HCLK <= 154 | 120
 < HCLK <= 140 |
             (++)
 |----------------|-------------------|-------------------|-------------------|-------------------|
             (++) |7WS(8CPU cycle) | 210 < HCLK <= 216 | 168 < HCLK <= 192 | 154 < HCLK <= 176 | 140
 < HCLK <= 160 |
             (++)
 |----------------|-------------------|-------------------|-------------------|-------------------|
             (++) |8WS(9CPU cycle) |        --         | 192 < HCLK <= 216 | 176 < HCLK <= 198 | 160
 < HCLK <= 180 |
             (++)
 |----------------|-------------------|-------------------|-------------------|-------------------|
...
/**
  * @brief  Update number of Flash wait states in line with new frequency and current
            voltage range.
  * @note   This Function support ONLY devices with supply voltage (voltage range) between 2.7V
  and 3.6V
    ...
    if (HCLK_Frequency == 0U) {
        ...
        if (LL_PWR_GetRegulVoltageScaling() == LL_PWR_REGU_VOLTAGE_SCALE1) {
            ...
            if (HCLK_Frequency > UTILS_SCALE2_LATENCY5_FREQ) {
                /* 150 < HCLK <= 168 OR 150 < HCLK <= 180 (when OverDrive mode is enable) => 5WS (6
                 * CPU cycles) */
                latency = LL_FLASH_LATENCY_5;
 ...
 *         - PLLM: ensure that the VCO input frequency ranges from 0.95 to 2.1 MHz (PLLVCO_input =
 * HSI frequency / PLLM)
 *         - PLLN: ensure that the VCO output frequency is between 100 and 432 MHz (PLLVCO_output =
 * PLLVCO_input * PLLN)
 ...
 *         - PLLM: ensure that the VCO input frequency ranges from 0.95 to 2.10 MHz (PLLVCO_input =
 * HSE frequency / PLLM)
 *         - PLLN: ensure that the VCO output frequency is between 100 and 432 MHz (PLLVCO_output =
 * PLLVCO_input * PLLN)
...
static uint32_t UTILS_GetPLLOutputFrequency(uint32_t PLL_InputFrequency,
    ...
    pllfreq = PLL_InputFrequency /
              (UTILS_PLLInitStruct->PLLM & (RCC_PLLCFGR_PLLM >> RCC_PLLCFGR_PLLM_Pos));
