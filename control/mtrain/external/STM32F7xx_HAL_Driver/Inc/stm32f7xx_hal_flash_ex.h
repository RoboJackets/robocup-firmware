...
typedef struct {
    ...
    uint32_t
        USERConfig; /*!< Program the FLASH User Option Byte: WWDG_SW / IWDG_SW / RST_STOP /
...
#define OB_DUAL_BOOT_ENABLE                                                                        \
    ((uint32_t)0x00000000U) /* !< Dual Boot enable. Boot always from system memory if boot address \
                               in flash (Dual bank Boot mode), or RAM if Boot address option in                                           \
                               RAM    */
/**
 * @}
 */
...
#define IS_OPTIONBYTE(VALUE)                                                \
    (((VALUE) <=                                                            \
      (OPTIONBYTE_WRP | OPTIONBYTE_RDP | OPTIONBYTE_USER | OPTIONBYTE_BOR | \
       OPTIONBYTE_BOOTADDR_0 | OPTIONBYTE_BOOTADDR_1 | OPTIONBYTE_PCROP | OPTIONBYTE_PCROP_RDP)))
#else
#define IS_OPTIONBYTE(VALUE)                                                           \
...
#if defined(FLASH_OPTCR_nDBANK)
#define IS_OB_NDBANK(VALUE) (((VALUE) == OB_NDBANK_SINGLE_BANK) || ((VALUE) == OB_NDBANK_DUAL_BANK))
...
#if defined(FLASH_OPTCR_nDBOOT)
#define IS_OB_NDBOOT(VALUE) (((VALUE) == OB_DUAL_BOOT_DISABLE) || ((VALUE) == OB_DUAL_BOOT_ENABLE))
#endif /* FLASH_OPTCR_nDBOOT */
