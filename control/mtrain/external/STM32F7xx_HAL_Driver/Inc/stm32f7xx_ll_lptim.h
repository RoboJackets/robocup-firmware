...
#define LL_LPTIM_COUNTER_MODE_EXTERNAL                                                          \
    LPTIM_CFGR_COUNTMODE /*!<The counter is incremented following each valid clock pulse on the \
...
#define LL_LPTIM_OUTPUT_WAVEFORM_PWM                                                            \
    0x00000000U /*!<LPTIM  generates either a PWM waveform or a One pulse waveform depending on \
                   chosen operating mode CONTINUOUS or SINGLE*/
#define LL_LPTIM_OUTPUT_WAVEFORM_SETONCE LPTIM_CFGR_WAVE /*!<LPTIM  generates a Set Once \
                                                            waveform*/
/**
 * @}
 */
...
#define LL_LPTIM_TRIG_FILTER_8                                                                    \
    LPTIM_CFGR_TRGFLT /*!<Trigger active level change must be stable for at least 8 clock periods \
...
#define LL_LPTIM_CLK_SOURCE_EXTERNAL                                                             \
    LPTIM_CFGR_CKSEL /*!<LPTIM is clocked by an external clock source through the LPTIM external \
                        Input1*/
/**
 * @}
 */
...
#define LL_LPTIM_CLK_FILTER_2                                                                     \
    LPTIM_CFGR_CKFLT_0 /*!<External clock signal level change must be stable for at least 2 clock \
                          periods before it is considered as valid transition*/
#define LL_LPTIM_CLK_FILTER_4                                                                     \
    LPTIM_CFGR_CKFLT_1 /*!<External clock signal level change must be stable for at least 4 clock \
                          periods before it is considered as valid transition*/
#define LL_LPTIM_CLK_FILTER_8                                                                   \
    LPTIM_CFGR_CKFLT /*!<External clock signal level change must be stable for at least 8 clock \
                        periods before it is considered as valid transition*/
/**
 * @}
 */
