...
#define TEMPSENSOR_CAL1_ADDR                                                                       \
    TEMPSENSOR_CAL1_ADDR_CMSIS /* Internal temperature sensor, address of parameter TS_CAL1: On    \
...
#define TEMPSENSOR_CAL2_ADDR                                                                       \
    TEMPSENSOR_CAL2_ADDR_CMSIS /* Internal temperature sensor, address of parameter TS_CAL2: On    \
                                  STM32F7, temperature sensor ADC raw data acquired at temperature \
...
#define LL_ADC_CHANNEL_VREFINT                                                                    \
    (LL_ADC_CHANNEL_17 |                                                                          \
     ADC_CHANNEL_ID_INTERNAL_CH) /*!< ADC internal channel connected to VrefInt: Internal voltage \
                                    reference. On STM32F7, ADC channel available only on ADC      \
                                    instance: ADC1. */
...
#define LL_ADC_REG_TRIG_EXT_TIM1_CH1                                                              \
    ((uint32_t)                                                                                   \
         ADC_REG_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group regular conversion trigger from external   \
                                           IP: TIM1 channel 1 event (capture compare: input       \
...
#define LL_ADC_REG_TRIG_EXT_TIM8_TRGO2                                                          \
    (ADC_CR2_EXTSEL_3 |                                                                         \
     ADC_REG_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group regular conversion trigger from external IP: \
                                       TIM8 TRGO2. Trigger edge set to rising edge (default     \
                                       setting). */
...
#define LL_ADC_REG_TRIG_EXT_TIM1_TRGO2                                                          \
    (ADC_CR2_EXTSEL_3 | ADC_CR2_EXTSEL_1 |                                                      \
     ADC_REG_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group regular conversion trigger from external IP: \
                                       TIM1 TRGO2. Trigger edge set to rising edge (default     \
                                       setting). */
...
#define LL_ADC_REG_TRIG_EXT_TIM6_TRGO                                                           \
    (ADC_CR2_EXTSEL_3 | ADC_CR2_EXTSEL_2 | ADC_CR2_EXTSEL_0 |                                   \
     ADC_REG_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group regular conversion trigger from external IP: \
                                       TIM6 TRGO. Trigger edge set to rising edge (default      \
                                       setting). */
...
#define LL_ADC_INJ_TRIG_EXT_TIM3_CH3                                                             \
    (ADC_CR2_JEXTSEL_3 | ADC_CR2_JEXTSEL_1 | ADC_CR2_JEXTSEL_0 |                                 \
     ADC_INJ_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group injected conversion trigger from external IP: \
                                       TIM3 channel 3 event (capture compare: input capture or   \
                                       output capture). Trigger edge set to rising edge (default \
                                       setting). */
...
#define LL_ADC_MULTI_REG_DMA_EACH_ADC                                                              \
    0x00000000U /*!< ADC multimode group regular conversions are transferred by DMA: each ADC uses \
                   its own DMA channel, with its individual DMA transfer settings */
 ...
 *
 *         (1) On STM32F7, parameter available only on ADC instance: ADC1.
 *         (2) On devices STM32F7x, limitation: this internal channel is shared between temperature
...
__STATIC_INLINE uint32_t LL_ADC_REG_GetSequencerRanks(ADC_TypeDef* ADCx, uint32_t Rank) {
    ...
    return (uint32_t)(
        READ_BIT(*preg, ADC_CHANNEL_ID_NUMBER_MASK << (Rank & ADC_REG_RANK_ID_SQRX_MASK)) >>
...
__STATIC_INLINE uint32_t LL_ADC_GetAnalogWDMonitChannels(ADC_TypeDef* ADCx) {
    return (uint32_t)(
        READ_BIT(ADCx->CR1, (ADC_CR1_AWDEN | ADC_CR1_JAWDEN | ADC_CR1_AWDSGL | ADC_CR1_AWDCH)));
}
